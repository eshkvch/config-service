...
- `DATABASE_URL` — полная строка подключения (опционально, если задана — имеет приоритет)
- `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, `DB_NAME` — альтернатива для Kubernetes (значения из Secret)
